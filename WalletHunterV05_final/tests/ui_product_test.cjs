...
test('live mark refresh preserves chart history and updates the current tail',()=>{assert.match(js,/function updateCandleTail\(rows,mark\)/);assert.match(js,/updateCandleTail\(S\.homeCandles,mark\)/);assert.doesNotMatch(js,/catch\{S\.candles=\[\];drawCharts\(\);\}/);});

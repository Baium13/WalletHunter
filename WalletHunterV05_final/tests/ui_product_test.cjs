...
test('operator review never labels arithmetic headroom as authorized capital',()=>{assert.match(js,/arithmetic_headroom/);assert.match(js,/NOT authorized trading capital/);assert.match(js,/available_for_new/);assert.match(js,/startBlocked=!!m\.quarantine/);assert.doesNotMatch(js,/fetch\([^\n]*enable-generation/);});
...
test('home chart keeps an isolated cache per account, network, mode and symbol',()=>{assert.match(js,/const homeMarketCache=new Map\(\)/);assert.match(js,/function homeMarketKey\(p\)/);assert.match(js,/scope\.tenant,scope\.account,scope\.network/);assert.match(js,/S\.homeCandles=cached\?\.candles\|\|\[\]/);assert.doesNotMatch(js,/dataset\.homePosition\)\{S\.homePositionId=b\.dataset\.homePosition;S\.homeCandles=\[\]/);});
test('account refresh runs in foreground independently of SSE deltas',()=>{assert.match(js,/setInterval\(\(\)=>\{if\(document\.hidden\|\|!S\.snapshot\)return;void refresh\(\);\},5000\)/);assert.match(js,/visibilitychange/);});
test('home percentage is labelled ROE because it is pnl divided by margin',()=>{assert.match(js,/metric\(t\('ROE %','ROE %'\)/);});

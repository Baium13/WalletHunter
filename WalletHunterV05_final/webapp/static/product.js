...
(() => {
  ...
  function home() {
    ...
    const summary = `<section class="product-section hero-section"><div class="product-card hero-card"><div class="hero-head"><div><span class="product-kicker">${esc(t('ОСНОВНОЙ СЧЁТ', 'PRIMARY ACCOUNT'))}</span><h2>${esc(state.manual?.configured ? t('Manual Copy', 'Manual Copy') : t('Wallet Hunter', 'Wallet Hunter'))}</h2></div><span class="product-badge ${healthy ? 'good' : 'warn'}">● ${esc(healthy ? t('АКТИВЕН', 'ACTIVE') : t('ПРОВЕРКА', 'CHECKING'))}</span></div><div class="hero-balance-label">${esc(t('БАЛАНС', 'BALANCE'))}</div><div class="hero-balance">${money(d.balance, true)}</div><div class="hero-pnl"><span>${esc(t('PnL', 'PnL'))}</span><b class="${(realized ?? 0) >= 0 ? 'product-good' : 'product-bad'}">${realized === null ? '—' : money(realized, true)}</b></div>${state.manual?.configured ? `<div class="hero-copy"><span>${esc(t('КОПИРУЕМЫЙ ЛИДЕР', 'COPY LEADER'))}</span><b>${esc(walletDisplay(state.manual.leader))}</b><small>${esc(t('Аллокация', 'Allocation'))}: ${esc(number(state.manual.allocation_pct, 0))}%</small></div>` : ''}</div><div class="product-grid kpis">${metric(t('Сделки', 'Trades'), number(report?.trades ?? d.trades, 0))}${metric(t('Win Rate', 'Win Rate'), report?.win_rate == null ? '—' : pct(Number(report.win_rate), 0))}${metric(t('Ожидание', 'Expectancy'), report?.expectancy == null ? '—' : money(report.expectancy, true))}</div></section>`;
    ...
    return `<div class="product-banner ${mode.cls}"><span><strong>${esc(mode.title)}</strong> · ${esc(mode.note)}</span><span class="product-badge ${healthy ? 'good' : 'warn'}">● ${esc(healthy ? t('СИСТЕМА В НОРМЕ', 'SYSTEM HEALTHY') : t('СОСТОЯНИЕ ПРОВЕРЯЕТСЯ', 'STATUS CHECK'))}</span></div>${summary}<section class="product-section chart-section">${card(t('Производительность', 'Performance'), performanceChart(report))}</section><section class="product-section"><div class="product-detail">${positionsCard}${leadersCard}</div></section><section class="product-section"><div class="product-detail">${consensusCard()}${manual}</div></section><section class="product-section">${activity}</section>`;
  }

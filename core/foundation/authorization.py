...
class AuthorizationPolicy(Contract):
    scope: Scope
    # LIVE_AUTO submits without a per-order human confirmation. It is the only
    # mode where policy alone is the signing authority, so it must be written
    # into the configuration deliberately; nothing promotes a policy into it.
    mode: Literal['OBSERVE','PAPER_AUTO','SHADOW','LIVE_CONFIRM','LIVE_AUTO']='OBSERVE'
...
class AuthorizationDecision(Contract):
    ...
    consensus_hash: Name
    policy_hash: Name
    mode: Literal['OBSERVE','PAPER_AUTO','SHADOW','LIVE_CONFIRM','LIVE_AUTO']
...
class AuthorizationService:
    ...
    def decide(self,policy,event,consensus,now):
        ...
        outcome,execution={'OBSERVE':('OBSERVE','NONE'),'PAPER_AUTO':('AUTHORIZED','PAPER'),
            'SHADOW':('HYPOTHETICAL','NONE'),'LIVE_CONFIRM':('CONFIRMATION_REQUIRED','NONE'),
            # The configured policy is the authority; there is no second human
            # step, so the decision is authorized for LIVE at the point it is
            # recorded. Every downstream gate (guard, Risk, ledger, gateway)
            # still applies exactly as it does for a confirmed order.
            'LIVE_AUTO':('AUTHORIZED','LIVE')}[policy.mode]
    ...
    def verify(self,decision,now):
        ...
        # A confirmation row exists only where a human had to add one. LIVE_AUTO
        # carries its authority in the immutable request itself, so keying the
        # table off execution_mode alone would look for a row that never exists.
        table='authorization_confirmations' if decision.mode=='LIVE_CONFIRM' else 'authorization_requests'
        with self.store.transaction() as db:

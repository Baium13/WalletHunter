...
class MarketSnapshot(Contract):
    ...
    source_version: Name
    # Resting notional on the thinner side of the top of book. Optional because
    # not every producer reads depth; a policy that requires it fails closed
    # when it is absent rather than treating "unknown" as "deep enough".
    depth_usd: Amount | None = None
...
class OrderIntent(Contract):
    ...
    authorization: Literal["USER_CONFIRMED", "COPY_POLICY", "PAPER_TEST", "PAPER_POLICY", "AUTONOMOUS_POLICY"]
    ...
    def coherent(self):
        ...
        if self.version == 1:
            ...
            # Live entry carries either a per-order human confirmation or a
            # standing autonomous policy. Nothing else may name execution_mode
            # LIVE at this version, and the shape is identical either way.
            if (self.authorization not in ('USER_CONFIRMED', 'AUTONOMOUS_POLICY') or self.execution_mode != 'LIVE'
                    or self.order_type not in ('IOC','LEVERAGE')
                    or self.source_contributions or self.parent_intent_id is not None):
                raise ValueError('Version 3 requires confirmed or autonomous live entry')
        elif self.version == 4:

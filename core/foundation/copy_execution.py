...
class HyperliquidExecutionAdapter:
    ...
    def validate_scope(self, intent):
        ...
        if intent.execution_mode != 'LIVE' or not (intent.authorization == 'COPY_POLICY' or
                ...
                # A standing LIVE_AUTO policy signs the same way a confirmed
                # order does. The grant it presents to the gateway still has to
                # match a durable authorization record, so this widens which
                # authority is accepted, not whether one is required.
                or (intent.version == 3 and intent.authorization == 'AUTONOMOUS_POLICY')):

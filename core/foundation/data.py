...
def reader_account_snapshot(reader, scope, revision, clock_ms):
    ...
    proves it.
    """
    from core.capital_snapshot import account_mode, finite_amount, strict_spot_usdc, UnsupportedCapitalMode
    ...
    # Same shared, bounded account-mode cache the capital reader uses: a
    # deliberate owner setting, not financial state, and 20 weighted units a
    # call. Balances below are still read fresh on every refresh.
    try:
        mode = account_mode(reader._info, scope.account, venue=reader.network)
    except UnsupportedCapitalMode as exc:
        mode = exc.mode

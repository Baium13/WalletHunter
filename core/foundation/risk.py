...
class RiskPolicy(Contract):
    ...
    # Liquidity bounds for entries. Slippage limits the price we will accept
    # and says nothing about whether the book can absorb the size; Manual Copy
    # reaches this gate without passing the agent layer, so without these two
    # a thin market is refused nowhere on that route. Zero disables a check,
    # which is the pre-existing behaviour for a policy that omits them.
    max_spread_bps: Amount = 0.
    minimum_depth_usd: Amount = 0.
...
def _liquidity(policy, market):
    """Entry-only book checks, as (code, ok) pairs. Empty when both are off.

    Neither is applied to REDUCE/CLOSE: refusing to shed risk because the book
    thinned is the opposite of what a liquidity guard is for.
    """
    checks = []
    if policy.max_spread_bps > 0:
        wide = (market.bid is None or market.ask is None
                or (market.ask - market.bid) / ((market.ask + market.bid) / 2) * 10000 > policy.max_spread_bps + 1e-9)
        checks.append(('SPREAD_LIMIT', not wide))
    if policy.minimum_depth_usd > 0:
        checks.append(('DEPTH_LIMIT', market.depth_usd is not None and market.depth_usd >= policy.minimum_depth_usd))
    return checks
...
class RiskGateway:
    ...
    def evaluate(self, intent, market, ledger, now, *, authorized=False, unresolved=False):
        ...
        require((intent.execution_mode in {"FAKE", "PAPER"} and intent.authorization == "PAPER_TEST")
            ...
            or (intent.version == 3 and intent.execution_mode == 'LIVE' and intent.authorization == 'USER_CONFIRMED')
            # Unattended live submission. It reaches this gate only from a
            # LIVE_AUTO authorization policy, and every check below - portfolio
            # freshness, ownership, leverage, exposure, capacity - is the same
            # one a confirmed order passes.
            or (intent.version == 3 and intent.execution_mode == 'LIVE' and intent.authorization == 'AUTONOMOUS_POLICY'),
            "LIVE_ROUTE_NOT_MIGRATED")
        reducing=intent.action in {'REDUCE','CLOSE'}
        require(intent.action in {'OPEN','ADD','REDUCE','CLOSE'}, "ACTION_NOT_MIGRATED")
        ...
        if not reducing: require(not ledger.errors, "LEDGER_RECONCILIATION_REQUIRED")
        require(not snapshot.orders, "OPEN_ORDERS_REQUIRE_RECONCILIATION")
        current=next((row for row in snapshot.positions if row.instrument==intent.instrument),None)
        if intent.action=='OPEN': require(current is None, "POSITION_ALREADY_EXISTS")
        else:
            require(current is not None and current.evidence=='VERIFIED' and current.margin is not None
                and bool(current.contributions) and all(c.source==intent.source for c in current.contributions), 'OWNERSHIP_UNPROVEN')
            if current:
                require((current.side=='LONG')==(intent.side=='BUY') if not reducing else (current.side=='LONG')!=(intent.side=='BUY'), 'ACTION_SIDE')
                require(intent.leverage==current.leverage, 'LEVERAGE_CHANGE_NOT_AUTHORIZED')
                if reducing: require(intent.size<=current.size and (intent.action!='CLOSE' or intent.size==current.size), 'REDUCTION_SIZE')
        if not reducing:
            require(intent.leverage <= p.max_leverage, "LEVERAGE_LIMIT")
            for code, ok in _liquidity(p, market): require(ok, code)
        ...
        try:
            ...
            if not reducing:
                ...
                require(notional+(current.notional if current else 0.) <= p.max_symbol_notional and total <= p.max_total_notional, "EXPOSURE_LIMIT")
    ...
    def _copy(self, intent, market, ledger, now, authorized, unresolved, policy=None):
        ...
        try:
            ...
            if reducing:
                ...
                for code, ok in _liquidity(p, market): check(ok, code)

...
class HyperliquidReader:
    ...
    def capital_snapshot(self, wallet):
        ...
        return read_capital_snapshot(self._info, wallet, venue=self.network)
...
class MarketResolvers:
    """Read-only market facts an autonomous backend needs but must not fetch.

    The backend deliberately owns no reader, so lot step, venue leverage
    ceiling and the leader's own running leverage arrive as plain callables.
    Nothing here can place, cancel or modify an order.

    Only the leader-leverage probe costs a request, and only when a decision is
    about to size an entry: a quiet leader is never polled. The answer is
    cached per leader and collateral pool, and it is charged at order priority
    because an entry is waiting on it.
    """

    def __init__(self, reader, ttl_seconds=60.0):
        self.reader = reader
        self.ttl = float(ttl_seconds)
        self._leverage = {}

    def lot_step(self, instrument):
        return self.reader.market_size_step(instrument.symbol, instrument.dex or "")

    def ceiling(self, instrument):
        return self.reader.leverage_limits(instrument.dex or "").get(instrument.symbol)

    def leader_leverage(self, wallet, instrument):
        from core.hl_budget import priority_scope
        dex = instrument.dex or ""
        key = (str(wallet).lower(), dex)
        now = time.time()
        hit = self._leverage.get(key)
        if hit is None or now - hit[0] >= self.ttl:
            with priority_scope("autonomous.leader_leverage", 1):
                state = self.reader.state(wallet, dex)
            rows = {}
            for p in HyperliquidReader._positions(state, "STOCKS" if dex else "CRYPTO", dex):
                try:
                    rows[str(p["coin"]).split(":")[-1]] = int(p["leverage"])
                except (KeyError, TypeError, ValueError):
                    pass
            if len(self._leverage) > 256:
                self._leverage.clear()
            hit = (now, rows)
            self._leverage[key] = hit
        return hit[1].get(instrument.symbol)

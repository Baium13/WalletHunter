...
def analyze(event, agents, evidence, now, policy):
    ...
    try:
        ...
        if m.ask is not None and m.bid is not None:
            ...
            # Per-market threshold, so a stock is not held to BTC's spread.
            spread_limit,_=policy.liquidity_for(event.instrument)
            check(math.isfinite(mid) and (m.ask-m.bid)/mid*10000<=spread_limit,'SPREAD_LIMIT')

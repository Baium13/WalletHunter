...
class TradeBuffer:
    ...
    def pressure(self):
        """How full the queue is, for the one consumer that may back off.

        Admitting new candidates is optional work; lifecycle reads are not.
        A caller uses this to pause admission before the buffer starts
        shedding, rather than after, since a drop is already lost data.
        """
        with self.condition:
            depth=len(self.normal)+len(self.critical)
            return dict(depth=depth,capacity=self.capacity,
                fraction=depth/max(1,self.capacity),dropped=self.dropped,
                protected_queued=len(self.critical))

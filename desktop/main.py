...
def _positions_url():
    """Open positions screen, served from the same origin as the Mini App.

    It must be launched BY Telegram to be usable: the page authenticates with
    the initData Telegram injects into a Web App launch, and a plain link
    opened in the in-app browser carries none, so the API would reject it.
    """
    from urllib.parse import urlsplit
    override = os.getenv("POSITIONS_URL")
    if override: return override
    parts = urlsplit(WEBAPP_URL)
    return f"{parts.scheme}://{parts.netloc}/static/positions.html?v=20260911-positions-1"
...
def dashboard_buttons(p):
    ...
    return [
        ...
        # A Web App button, not an inline callback: Telegram opens the page
        # itself and supplies the signed initData the read API requires.
        [KeyboardButtonWebView("📂 Позиции · PAPER / LIVE", POSITIONS_URL)],
    ]
...
async def positions(event):
    ...
    await event.respond("Открытые позиции: симулированные PAPER и ваши реальные, с переключением.",
        buttons=[[KeyboardButtonWebView("📂 Открыть позиции", POSITIONS_URL)]], parse_mode=None)
...
# --- Unattended live consumer -------------------------------------------------
# The research worker holds no credentials by design, so a LIVE_AUTO consumer of
# its decisions has to run here, where the signing client already exists. All
# three variables must be set: with any of them absent the loop never starts and
# this service behaves exactly as it did before.
LIVE_AUTO_CONFIG = os.getenv("LIVE_AUTO_CONFIG")
LIVE_AUTO_STATE = os.getenv("LIVE_AUTO_STATE_DIRECTORY")
LIVE_AUTO_RESEARCH = os.getenv("LIVE_AUTO_RESEARCH_DB")
LIVE_AUTO_INTERVAL = max(5.0, float(os.getenv("LIVE_AUTO_INTERVAL", "15") or 15))
_live_auto = {}


def _live_auto_ready():
    """Build the live consumer once, bound to the profile that owns its scope."""
    if _live_auto.get("backend"): return _live_auto["backend"], _live_auto["engine"]
    ...
    with open(LIVE_AUTO_CONFIG, encoding="utf-8") as handle: config = jsonlib.load(handle)
    declared = config["authorization"]["scope"]
    if config["authorization"].get("mode") != "LIVE_AUTO":
        raise ValueError("LIVE_AUTO_CONFIG is not a LIVE_AUTO configuration")
    tenant, address = str(declared["tenant"]), str(declared["account"]).lower()
    _, p = store.profile(int(tenant))
    if (p.get("account") or {}).get("address", "").lower() != address:
        raise ValueError("Configured scope does not match that profile's account")
    c = account_client(int(tenant), p)
    scope = Scope(tenant=tenant, account=address, network=declared["network"])
    clock = lambda: int(time.time() * 1000)

    def evidence(revision, dex):
        """Fresh exchange portfolio for the collateral pool this instrument uses."""
        return copy_account_snapshot(c, scope, revision, clock, dex or "")

    from core.hyperliquid import MarketResolvers
    markets = MarketResolvers(reader)

    opening = evidence(1, "")
    backend = load_paper_backend(LIVE_AUTO_CONFIG, LIVE_AUTO_STATE, S.hl_mode, clock,
        precision=markets.lot_step, ceilings=markets.ceiling, leader_leverage=markets.leader_leverage,
        exchange=HyperliquidExecutionAdapter(c, scope, clock, opening), evidence=evidence)
    with backend.store.transaction() as db:
        if not db.execute("SELECT 1 FROM portfolios WHERE scope=?", (scope_key(scope),)).fetchone():
            # A live scope opens on exchange evidence, never on a configured number.
            backend.store.publish_portfolio_in(db, opening, "live-auto-open")
    engine = WalletDiscoveryEngine(LIVE_AUTO_RESEARCH, S.hl_mode)
    _live_auto.update(backend=backend, engine=engine)
    print("[LIVE AUTO] consumer ready for", short_wallet(address), flush=True)
    return backend, engine


async def autonomous_live_watcher():
    """Consume research decisions into live orders under the configured guard."""
    if not (LIVE_AUTO_CONFIG and LIVE_AUTO_STATE and LIVE_AUTO_RESEARCH): return
    while True:
        try:
            backend, engine = await asyncio.to_thread(_live_auto_ready)
            await asyncio.to_thread(backend.drain, engine)
        except Exception as exc:
            # Type only. A raw exception from any Telegram or HTTP path carries
            # the request URL, and a bot API URL has the token inside it; this
            # loop logs on every failure, so one leak would repeat forever.
            print("[LIVE AUTO]", type(exc).__name__, flush=True)
        await asyncio.sleep(LIVE_AUTO_INTERVAL)
...
async def main():
    ...
    asyncio.create_task(autonomous_live_watcher())

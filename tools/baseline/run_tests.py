...
def argument_parser():
    ...
    parser.add_argument("--phase",choices=("0", "1.1", "1.2", "1", "block1"),default="0")
...
def validate_options(args, source):
    ...
    if args.phase in {"1.1", "1.2", "1", "block1"}:
        ...
        if args.phase == "block1":
            historical.update(REPOSITORY / "docs" / name for name in ("p1.1-validation.json", "p1.2-validation.json", "phase1-validation.json"))
...
def main():
    ...
    if args.phase != "0":
        report.update(evidence_label=("P1.1 authorized ownership-history cache validation" if args.phase == "1.1"
                                     ...
                                     else "P1.3-P1.10 authorized stabilization validation" if args.phase == "1"
                                     else "Block 1 additive core validation"),

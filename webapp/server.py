...
account_snapshot_cache: dict[tuple[str, str, str], tuple[float, object]] = BoundedCache(64)
_account_snapshot_lock = threading.RLock()
ACCOUNT_SNAPSHOT_TTL = 5.0
...
# Read-only window on the autonomous runtimes. Each entry is a state directory
# an operator has actually configured; an unset variable simply means that
# runtime is not shown, never that the endpoint is missing.
AUTONOMY_RUNTIMES = {name: os.environ[variable] for name, variable in
                     (('PAPER', 'PAPER_STATE_DIRECTORY'), ('SHADOW', 'SHADOW_STATE_DIRECTORY'),
                      ('LIVE', 'LIVE_AUTO_STATE_DIRECTORY'))
                     if os.environ.get(variable)}
from webapp.autonomy_api import router as autonomy_router
app.include_router(autonomy_router(AUTONOMY_RUNTIMES, settings.hl_mode, require_user,
                                   budget_path=os.getenv('HL_API_BUDGET_DB')
                                   or os.path.join(ROOT, 'data', 'hl-api-budget.sqlite3')))
